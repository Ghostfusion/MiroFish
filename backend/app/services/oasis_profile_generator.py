...
from ..utils.graph_client import normalize_search_query, search_edges, search_nodes
...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # 图谱检索用于丰富上下文，读取统一走进程内 graph_client
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Returns:
            包含facts, node_summaries, context的字典；未设置graph_id时返回空字典
        """
        ...
        comprehensive_query = normalize_search_query(
            t('progress.graphSearchQuery', name=entity_name)
        )
        
        def search_entity_edges():
            """搜索边（事实/关系）"""
            return search_edges(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            edge_future = executor.submit(search_entity_edges)
            node_future = executor.submit(search_entity_nodes)
            
            # 获取结果（检索失败会在此抛出，交由调用方按实体降级处理）
            edge_result = edge_future.result()
            node_result = node_future.result()
        
        # 处理边搜索结果
        all_facts = set()
        for edge in edge_result or []:
            if edge.get("fact"):
